...
use crate::cal::{Expiration, MessageBuffer, Reliability, TimeBasedFilter, TopicQos};
use crate::calconfig::{CalConfig, ReliabilityConfig, TopicDirection};
...
pub(crate) fn apply_config_qos(
    ...
    qos.time_based_filter = qos.time_based_filter.or_else(|| {
        cfg.time_based_filter_ms.map(|ms| TimeBasedFilter {
            min_separation: std::time::Duration::from_millis(ms),
        })
    });
    qos.expiration = qos.expiration.or_else(|| {
        cfg.expiration_ms.map(|ms| Expiration {
            max_age: std::time::Duration::from_millis(ms),
        })
    });
