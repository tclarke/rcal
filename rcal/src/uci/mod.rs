...
/// Render a US classification banner string from a [`SecurityInformationType`](types::SecurityInformationType).
///
...
pub fn security_banner(s: &dyn types::SecurityInformationType) -> String {
    use types::ReleasableToChoiceType_;

    let class_str = s.classification().as_str().unwrap_or("U");
    let banner_class = if !s.cuibasic().is_empty() && class_str == "U" {
        "CUI"
    } else {
        class_str
    };
    ...
    let sci: Vec<&str> = s.scicontrols().iter().filter_map(|v| v.as_str()).collect();
    if !sci.is_empty() {
        banner.push_str("//");
